# Graph Report - land-58  (2026-09-20)

## Corpus Check
- 242 files · ~220,882 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2162 nodes · 4574 edges · 155 communities (134 shown, 21 thin omitted)
- Extraction: 83% EXTRACTED · 17% INFERRED · 0% AMBIGUOUS · INFERRED: 761 edges (avg confidence: 0.51)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `aed4ad33`
...
1. `PersistenceService` - 91 edges
2. `AuditContext` - 82 edges
3. `Category` - 75 edges
4. `ReceiptInput` - 54 edges
5. `ReconciliationOutcome` - 51 edges
6. `PersistedReceipt` - 47 edges
7. `ExpectedShipmentInput` - 43 edges
8. `AttachmentInput` - 41 edges
9. `Status` - 40 edges
10. `ComparedField` - 40 edges
...
Cohesion: 0.08
Nodes (25): CaseReviewTarget, ReconciliationExceptionActionType, ReconciliationExceptionReviewTarget, ReviewAssignment, ReviewAssignmentState, ReviewHistoryEntry, PREPARED_FIXTURES, ACTION_LABEL (+17 more)
...
### Community 30 - "react"
Cohesion: 0.19
Nodes (9): App(), ThemeSeed(), HeroFilm(), renderTable(), renderView(), applyTheme(), AppRoutes(), renderAt() (+1 more)
...
Cohesion: 0.12
Nodes (20): ExpectedShipment, EXPECTED_SHIPMENTS_CSV_HEADER, parseExpectedShipmentsCsv(), REQUIRED_DOCUMENTS, SOURCE_FRESHNESS, stableHash(), PREPARED_RUN_ID, deriveReconciliationResults() (+12 more)

### Community 75 - "Domain.tsx"
Cohesion: 0.11
Nodes (21): DropZone(), DropZoneProps, FieldRow(), FieldRowProps, formatCeiling(), ProvenanceAnchor(), ProvenanceAnchorProps, ScrollbarProps (+13 more)
...
Cohesion: 0.14
Nodes (81): Category, ComparedField, EvaluatorOutput, ExtractionResult, FieldVerdict, StrEnum, ReconciliationOutcome, ReconciliationResult (+73 more)

### Community 79 - "Controls.tsx"
Cohesion: 0.11
Nodes (14): ButtonProps, Checkbox(), CheckboxProps, CheckboxState, Field(), FieldControlProps, FieldProps, FieldTriggerProps (+6 more)

### Community 80 - "test_persistence.py"
Cohesion: 0.09
Nodes (63): GcsPrivateObjectStore, InMemoryPrivateObjectStore, private_object_key(), Client, sha256_hex(), _validate_hash(), _async_database_url(), postgres_engine() (+55 more)
...
Cohesion: 0.12
Nodes (19): Scrollbar(), Tooltip(), CsvImportSection(), CsvImportSectionProps, ExpectedShipmentTable(), ExpectedShipmentTableProps, FRESHNESS_KIND, FRESHNESS_LABEL (+11 more)
...
Cohesion: 0.16
Nodes (7): MissingCasePeakCard(), MissingCasePeakCardProps, OUTCOMES, parsed, PREPARED_DATASET_LABEL, PREPARED_EXPECTED_SHIPMENTS, PREPARED_RECONCILIATION_RESULTS

### Community 90 - "Overlays.tsx"
Cohesion: 0.09
Nodes (20): GlyphProps, stroke, VerdictCheckGlyph(), addMonths(), DatePicker(), DatePickerProps, Menu(), MenuItem() (+12 more)
...
Cohesion: 0.06
Nodes (50): AmbiguousReconciliation, _canonical_hash(), _canonical_ids(), compute_subject_key(), _ContractModel, DigitalPdfLocation, DigitalPdfProvenance, DocxParagraphLocation (+42 more)
...
Cohesion: 0.13
Nodes (30): BundleEmail, _canonical_message_bytes(), BaseModel, Validate and read an organizer inbox without inventing source timestamps., read_bundle(), _load_organizer_inbox(), LocalInbox, MemoryInbox (+22 more)
...
Cohesion: 0.27
Nodes (8): VerdictHoldGlyph(), ReviewReason, ACTIONABLE_DISPOSITIONS, HeldReviewCard(), HeldReviewCardProps, REVIEW_REASON_LABELS, SETTLED_STATUS, CaseReviewDetails

### Community 99 - "JevClassification"
Cohesion: 0.13
...
### Community 108 - "JevProviderFailure"
Cohesion: 0.16
Nodes (19): _answer_fields(), AsyncSystemOneClient, AttachmentLike, ClassifiableEmail, _EmailState, JevProviderFailure, _nonempty_string(), _parse_answer() (+11 more)
...
### Community 111 - "ReviewQueueTable.tsx"
Cohesion: 0.22
...
Cohesion: 0.29
Nodes (18): pytest_collection_modifyitems(), async_sessionmaker, asyncio, AsyncSession, parametrize, postgres, UUID, _seed_append_only_rows() (+10 more)
...
Cohesion: 0.19
Nodes (10): _check_constraint(), test_expected_shipments_retain_typed_reconciliation_inputs(), test_idempotency_key_and_guest_generation_are_scoped(), test_receipt_uniqueness_uses_scoped_partial_indexes(), test_reconciliation_results_enforce_discriminated_outcome_shapes(), test_source_objects_are_content_addressed_and_private(), test_trd_cache_and_domain_uniqueness_constraints_are_present(), _unique_constraint_columns() (+2 more)
...
Cohesion: 0.37
Nodes (8): Button(), AppShell(), isTheme(), readTheme(), Theme, toggleTheme(), AuthPage(), LandingPage()

### Community 121 - "guest-session.ts"
Cohesion: 0.35
Nodes (7): clearGuestSession(), createGuestSession(), ensureGuestSession(), GuestSession, readGuestSession(), JudgePage(), OperatorGuard()

### Community 122 - "routes.tsx"
Cohesion: 0.27
Nodes (3): SiteFooter(), SiteShell(), PlaceholderView()
...
### Community 135 - "SDOC hackathon — delivery kit"
Cohesion: 0.29
...
### Community 144 - "_typesafe_sdk_retry_policy"
Cohesion: 0.50
Nodes (4): fixture, MonkeyPatch, _typesafe_sdk_retry_policy(), SimpleNamespace
...
## Knowledge Gaps
- **835 isolated node(s):** `$schema`, `printWidth`, `singleQuote`, `semi`, `trailingComma` (+830 more)
...
- **Why does `react` connect `react` to `plugins`, `HeldReviewCard.tsx`, `routes.tsx`, `AccessibleGraphTable.tsx`, `EmailDetailView.tsx`, `Domain.tsx`, `EvidenceViewer.tsx`, `Controls.tsx`, `ReviewQueueTable.tsx`, `ReconciliationView.tsx`, `Button`, `review-queue/types.ts`, `Overlays.tsx`, `ReviewPage.tsx`, `EvaluationPage.tsx`, `InboxPage.tsx`?**
  _High betweenness centrality (0.019) - this node is a cross-community bridge._
- **Why does `Category` connect `PersistenceService` to `read_bundle`, `JevClassification`, `ingestion.py`, `test_jev.py`, `JevProviderFailure`, `contracts.py`?**
  _High betweenness centrality (0.014) - this node is a cross-community bridge._
- **Why does `AuditContext` connect `PersistenceService` to `read_bundle`, `JevClassification`, `ingestion.py`, `test_persistence.py`, `reconciliation.py`?**
  _High betweenness centrality (0.013) - this node is a cross-community bridge._
- **Are the 33 inferred relationships involving `PersistenceService` (e.g. with `Category` and `ComparedField`) actually correct?**
  _`PersistenceService` has 33 INFERRED edges - model-reasoned connections that need verification._
- **Are the 48 inferred relationships involving `AuditContext` (e.g. with `AttachmentReceipt` and `BundleEmail`) actually correct?**
  _`AuditContext` has 48 INFERRED edges - model-reasoned connections that need verification._
- **Are the 65 inferred relationships involving `Category` (e.g. with `AttachmentReceipt` and `BundleEmail`) actually correct?**
  _`Category` has 65 INFERRED edges - model-reasoned connections that need verification._
- **Are the 41 inferred relationships involving `ReceiptInput` (e.g. with `AttachmentReceipt` and `BundleEmail`) actually correct?**
  _`ReceiptInput` has 41 INFERRED edges - model-reasoned connections that need verification._