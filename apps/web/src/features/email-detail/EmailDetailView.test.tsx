...
describe('EmailDetailView acceptance behaviors', () => {
  ...
  it('2. Mismatch/held rows expose status via accessible text and glyph, not color only', async () => {
    const service = createPreparedEmailDetailService()
    ...
    expect(row.querySelector('.field-row-rail')).toBeNull()
  ...
  it('3. Provenance actions open/jump to format-honest evidence for TXT line/column, digital PDF page/bbox, XLSX sheet/cell, DOCX table/paragraph, scanned approximate region, and corrupt none', async () => {
    ...
    // TXT line and column
    await user.click(screen.getByRole('button', { name: 'TXT_VALUE_PT_INDAH' }))
    let evidence = screen.getByRole('region', { name: 'Source evidence' })
    expect(within(evidence).getByText('manifest.txt')).toBeInTheDocument()
    expect(within(evidence).getByText(/Line 10, columns 5 to 25/)).toBeInTheDocument()

    // Digital PDF page and bounding box
    await user.click(screen.getByRole('button', { name: 'PDF_DIGITAL_VALUE' }))
    evidence = screen.getByRole('region', { name: 'Source evidence' })
    expect(within(evidence).getByText('bill_digital.pdf')).toBeInTheDocument()
    expect(within(evidence).getByText(/Page 1/)).toBeInTheDocument()
    expect(within(evidence).getByText(/72\.0, 140\.0, 280\.0, 165\.0/)).toBeInTheDocument()

    // XLSX sheet and cell
    await user.click(screen.getByRole('button', { name: 'XLSX_VALUE_BALL_DOGGETT' }))
    evidence = screen.getByRole('region', { name: 'Source evidence' })
    expect(within(evidence).getByText('booking_sheet.xlsx')).toBeInTheDocument()
    expect(within(evidence).getByText(/Sheet S\.I\., Cell B5/)).toBeInTheDocument()

    // DOCX table cell
    await user.click(screen.getByRole('button', { name: 'DOCX_VALUE_TABLE_CELL' }))
    evidence = screen.getByRole('region', { name: 'Source evidence' })
    expect(within(evidence).getByText('draft_bl.docx')).toBeInTheDocument()
    expect(within(evidence).getByText(/Table 0, row 1, column 1/i)).toBeInTheDocument()

    // DOCX paragraph
    await user.click(screen.getByRole('button', { name: 'DOCX_PARAGRAPH_VALUE' }))
    evidence = screen.getByRole('region', { name: 'Source evidence' })
    expect(within(evidence).getByText(/Paragraph 2/)).toBeInTheDocument()

    // Scanned PDF approximate region
    await user.click(screen.getByRole('button', { name: 'SCAN_VALUE_APPROX_REGION' }))
    evidence = screen.getByRole('region', { name: 'Source evidence' })
    ...
    expect(within(evidence).getByText(/Cargo region/i)).toBeInTheDocument()

    // Corrupt attachment: value shown without a usable anchor
    const loadingRow = screen.getByText('Port of loading').closest('.field-row') as HTMLElement
    expect(within(loadingRow).getByText('Unreadable')).toBeInTheDocument()
    expect(within(loadingRow).getByText('No source anchor')).toBeInTheDocument()
    expect(within(loadingRow).queryByRole('button', { name: /Unreadable/ })).not.toBeInTheDocument()
  })
  ...
  it('4. email_507 shows missing attachment refusal with zero FieldRows', async () => {
    ...
    expect(screen.queryByRole('region', { name: 'Source evidence' })).not.toBeInTheDocument()
    expect(screen.queryByText(/Click any compared field value/i)).not.toBeInTheDocument()
  })

  it('4b. The attachments metadata counts only files actually present, not missing-required placeholders', async () => {
    const service = createPreparedEmailDetailService()
    const { rerender } = render(<EmailDetailView emailId="email_001" service={service} />)
  ...
  it('6. Approve/correct/reject go through the review seam and refresh the rendered history/owner; exactly one primary button is present', async () => {
    ...
    expect(await screen.findByText(/Operator sign-off confirmed/i)).toBeInTheDocument()
  })
  ...
  it('6c. A provenance jump scrolls the source evidence region into view', async () => {
    ...
    try {
      ...
      await screen.findByText('Prepared record')
      await user.click(screen.getAllByRole('button', { name: /MOORIM SP CO\., LTD/i })[0])
  ...
  it('8a. Prepared record copy reads plainly without implementation jargon', async () => {
    ...
    await user.hover(screen.getByRole('button', { name: 'About prepared data' }))
  ...
  it('8. Changing emailId returns the view to a loading state before showing the next record', async () => {
    ...
    expect(await screen.findByText(/Refusal: Missing required draft bill of lading/i)).toBeInTheDocument()
  })
