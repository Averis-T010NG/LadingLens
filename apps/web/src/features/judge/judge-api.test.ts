...
describe('judge API client', () => {
  ...
  it('submits both files and the confirmation flag as multipart form data without a manual content type', async () => {
    ...
    // Either order: the check tells the SI from the draft BL by what they say.
    const bl = file('bl.txt')
    const si = file('si.txt')
    await expect(createJudgeRun({ files: [bl, si], confirmed: true })).resolves.toEqual(RUN_SUCCEEDED)

    expect(capturedInit?.method).toBe('POST')
    expect(capturedInit?.body).toBeInstanceOf(FormData)
    const form = capturedInit?.body as FormData
    expect(form.getAll('files')).toEqual([bl, si])
    expect(form.has('si_file')).toBe(false)
  ...
  it('maps a 422 upload rejection into a JudgeUploadError carrying the slot rejections', async () => {
      ...
      vi.fn(async () =>
        json(422, {
          error: {
            ...
            details: [{ slot: 'file_1', reason: 'unsupported_format' }]
          }
        })
      )
    )
    ...
    const error = await createJudgeRun({
      files: [file('si.png', 'x', 'image/png'), file('bl.txt')],
      confirmed: true
    ...
    expect(error).toMatchObject({
      ...
      code: 'upload_rejected',
      message: 'One or more files could not be used.',
      rejections: [{ slot: 'file_1', reason: 'unsupported_format' }]
    })
  })
